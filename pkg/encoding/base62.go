...
// Encoder defines methods for encoding and decoding int64 to/from base62 strings.
//
//go:generate mockery --name Encoder --filename base62.go --output ./mocks
type Encoder interface {
	EncodeInt64ToString(num int64) (string, error)
	DecodeStringToInt64(s string) (int64, error)
}
...
//
// Returns:
//   - An Encoder instance configured with the provided alphabet.
//
// Panics:
//   - If the alphabet length is not 62.
//   - If the alphabet contains duplicate characters.
func NewEncoding(alphabet string) Encoder {
	if len(alphabet) != 62 {
